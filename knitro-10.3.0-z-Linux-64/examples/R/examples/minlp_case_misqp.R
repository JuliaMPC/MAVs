...
#
# Solving MIP using knitromip with new MISQP algorithm
#
...
# Set knitro options
knitro_opts <- list("gradopt"=2,
                    "hessopt" = 2,
                    "mip_method" = 3)

# Call Knitro MINLP solver
# In this example, we provide the jacobian but we do not use it (gradopt
# is set to KTR_GRADOPT_FORWARD=2 for forward finite differences).
# MISQP algorithm usually works better with finite differences.
mip_sol <- knitromip(objective = eval_f,
                     ...
                     xType = xType,
                     cFnType = cFnType,
                     constraintsTypes = c(0, 0, 1, 1, 1, 1),
                     objfntype = objfntype,
                     options = knitro_opts)
