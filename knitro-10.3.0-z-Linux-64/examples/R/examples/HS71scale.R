...
# with scaling on variables, constraints and objective
#
...
# Create new R environment and set to options
opts <- list("environ"=new.env(),
             "derivcheck"=1,
             "derivcheck_tol"=0.03,
             "outlev"=6,
			 "outmode"=1)

# Scale variables, constraints and objective
xScaleFactors = 5000 * c(1.0, 1.0, 1.0, 1.0)
xScaleCenters = c(0.1, 0.0, 0.2, 0.0)
cScaleFactors = c(100.0, 10000)
objScaleFactor = 1.0
...
sol <- knitro(x0 = x0,
			  ...
			  xScaleCenters=xScaleCenters,
			  cScaleFactors=cScaleFactors,
		      objScaleFactor=objScaleFactor,
			  options = opts)
